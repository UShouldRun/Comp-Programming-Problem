...
The makefile uses the following flags for robust compilation:

**Standard and Optimization:**
- `-std=c++20`: Use C++20 standard (required for designated initializers)
- `-O2`: Level 2 optimization (balanced speed/compile time)
...
**Test file format**: Place input files with `.in` extension in the `test/` directory.

## Author

**Henrique Teixeira**

Competitive Programming Course Assignment
Topics: Graph Algorithms, Maximum Flow, Shortest Paths
...
Since warehouse 1 has capacity 1, it can only serve one store. The algorithm chooses to serve store 2, leaving store 3 unserved.
...
**Key Insight:** This demonstrates that when multiple warehouses can serve the same store, the algorithm selects the one with minimum cost.
